...
podman run --rm cyberark/conjur data-key generate > data_key
echo
...
podman-compose exec -T conjur conjurctl wait -r 120 -p 80
echo

echo "Step 5: Create admin account"
podman-compose exec -T conjur conjurctl account create myConjurAccount > admin_data
echo

echo "Step 6: Connect the Conjur client to the Conjur server"
# `echo "Y"` is used to accept the self-signed certificate
echo "Y" | podman container exec -i conjur_client conjur init -u https://proxy -a myConjurAccount --self-signed
echo
...
trap ''  EXIT ABRT QUIT
command="podman exec --interactive conjur_client conjur login -i admin -p ${admin_api_key}"
...
function loginAsAdmin {
    ...
    while [ $retries -lt $max_retries ]; do
        ...
        echo "Attempt $retries: Logging in as admin..."
    ...
    echo "Maximum retries reached. Failed to log in as admin."
    return 1
}

# Call the login function
if ! loginAsAdmin; then
    echo "Failsafe: Unable to log in as admin"
    exit 1
fi
...
podman-compose exec -T client conjur policy load -b root -f policy/BotApp.yml > my_app_data
echo

echo "Step 3: Log out of Conjur as admin"
podman-compose exec -T client conjur logout
echo
...
podman-compose exec -T client conjur login -i Dave@BotApp -p ${dave_api_key}
echo
...
podman-compose exec -T client conjur variable set -i BotApp/secretVar -v ${secretVal}
echo
...
podman-compose exec -T bot_app bash -c "curl -d "${bot_api_key}" -k https://proxy/authn/myConjurAccount/host%2FBotApp%2FmyDemoApp/authenticate > /tmp/conjur_token"
echo

echo "Step 3: Fetch Secret"
fetched=$(podman-compose exec -T bot_app bash -c "/tmp/program.sh")
echo
